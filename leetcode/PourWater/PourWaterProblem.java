...
public class PourWaterProblem {
    ...
    public int pourWater(int capacity1, int capacity2, int target) {
        if (capacity1 < target && capacity2 < target) {
        ...
        while (!queue.isEmpty()) {
            ...
            while (size-- > 0) {
                ...
                List<State> nexts = generateNextStates(cur, visited, capacity1, capacity2);
                queue.addAll(nexts);
            }
            numSteps++;
        }
